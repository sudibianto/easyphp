...
$serverconf = file_get_contents(__DIR__ . '\conf\nginx.conf');
...
<div class="row">
	...
	<div class="col-sm-11">
		...
		<p>
			&#9642; version : <b><?php echo $app_settings['app_version'] ?></b></br>
		</p>
	</div>
</div>
			...
			<tbody>
				...
				if ($eds_httpserver_running == 1) {
					...
					<tr>
						...
						<td colspan="2">
							<?php
							preg_match('/^[\s]+server_name(.*);(.*)$/m', $serverconf, $serverhost);
							preg_match('/^[\s]+listen(.*);(.*)$/m', $serverconf, $serverport);
							...
							&nbsp;&nbsp;&nbsp;&nbsp;<em>Port: </em><kbd><?php echo trim($serverport[1]); ?></kbd>
							&nbsp;&nbsp;&nbsp;&nbsp;<em>Host: </em><kbd><?php echo trim($serverhost[1]); ?></kbd>
					...
					<tr>
						...
						<td style="white-space:nowrap;">
							<a href="http://<?php echo trim($serverhost[1]) . ':' . trim($serverport[1]); ?>" data-toggle="tooltip" data-placement="top" title="Browse http://<?php echo trim($serverhost[1]) . ':' . trim($serverport[1]) ?>" target="_blank" class="btn btn-default btn-xs"><span class="glyphicon glyphicon-link small" aria-hidden="true"></span></a>
						</td>
						<td>
							<samp style="color:silver;" class="small">http://<?php echo trim($serverhost[1]) . ':' . trim($serverport[1]); ?></samp>
						</td>
					</tr>
					<tr>
						...
						<td style="white-space:nowrap;">
							<?php			
							preg_match('/^[\s|\t]*root[\s|\t]+\"(.*)\";(.*)$/m', $serverconf, $documentroot);
	...
	if (isset($_GET['display']) AND $_GET['display'] == 'serverconffile') {
		...
		<div style="float:right;padding-bottom:10px;">
			<form method="post" style="display:inline" action="http://<?php echo $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'] ?>" class="form-inline" role="form">
				...
				<input type="hidden" name="action[request][0][value]" value="<?php echo urlencode('explorer ' . __DIR__ . '\conf\nginx.conf') ?>" />
