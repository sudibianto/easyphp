...
#        handle -lmariadb  ws2_32 mariadbclient advapi32 kernel32 shlwapi crypt32 secur32   and other strings inserted that might contain
...
$flags->{libs} =
  [@ldflags,@lib_opts,'','','',''];
$flags->{libs_r} =
  [@ldflags,@lib_r_opts,'','-lmariadb  ws2_32 mariadbclient advapi32 kernel32 shlwapi crypt32 secur32  ',''];
$flags->{embedded_libs} =
  [@ldflags,@lib_e_opts,'','','-lmariadb  ws2_32 mariadbclient advapi32 kernel32 shlwapi crypt32 secur32  ','',''];
